...
import { isFiniteWorkspaceNumber, MAX_WORKSPACE_NUMBER, singularizeTerm, uid } from "@/lib/utils";
...
export function QuestForm({ goalId, open, onClose, quest }: { goalId: string; open: boolean; onClose: () => void; quest?: Quest }) {
  ...
  return <Modal open={open} onClose={close} eyebrow="Next useful action" title={quest ? `Edit ${questTerm.toLowerCase()}` : `Create a ${questTerm.toLowerCase()}`}>
    <div className="form-stack">
      <Field label={`${questTerm} title`}><input data-modal-autofocus="true" maxLength={WORKSPACE_TEXT_LIMITS.actionTitle} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Complete Spanish lesson 12" /></Field>
      ...
      <Field label={`${questTerm} type`} hint="Choose the shape of the action, independently of its schedule"><select value={kind} onChange={(e) => setKind(e.target.value as Quest["kind"])}><option value="task">Task</option><option value="session">Session</option><option value="challenge">Challenge</option><option value="milestone">{singularizeTerm(terms.milestones)} action</option></select></Field>
      ...
      <div className="setting-note"><Plus size={17} /><p>When completed, this action will be recorded in the {goalTerm.toLowerCase()} history with its date and any linked metric change.</p></div>
      <div className="button-row end"><Button variant="ghost" onClick={close}>Cancel</Button><Button disabled={!title.trim() || !durationValid || !metricAmountValid} onClick={submit}><Plus size={16} /> {quest ? `Save ${questTerm.toLowerCase()}` : `Add ${questTerm.toLowerCase()}`}</Button></div>
    </div>
  </Modal>;
}
