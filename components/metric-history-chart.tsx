...
export function MetricHistoryChart({ metric, entries, color }: { metric: ProgressMetric; entries: MetricEntry[]; color?: string }) {
  ...
  const summary = data.map((item) => `${item.label}: ${item.value} ${item.unit}`).join(", ");
  ...
  return (
    <section className="metric-history-block">
      <div><strong>{metric.label} history</strong><small>{mixedUnits ? "Recorded units changed; values are listed without a cross-unit trend" : `Last ${data.length} recorded changes`}</small></div>
