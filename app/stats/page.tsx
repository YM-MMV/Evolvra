...
import { formatDate, localDateKey, singularizeTerm } from "@/lib/utils";
...
export default function StatsPage() {
  ...
  const historySummary = visibleStats.length
    ...
    : "No current qualities are available.";
  ...
  return <div>
    <section className="page-header"><div><p className="eyebrow">Qualities connected to your {terms.goals.toLowerCase()}</p><h1>Your {terms.stats.toLowerCase()}</h1><p className="page-lead">See how permanent action, {terms.milestones.toLowerCase()}, measurement, and check-in records connect to the qualities you chose for each {goalTerm.toLowerCase()}.</p></div><div className="filter-tabs" role="group" aria-label="Analytics time window">{([30, 90, 365] as const).map((days) => <button key={days} type="button" className={windowDays === days ? "active" : ""} aria-pressed={windowDays === days} onClick={() => setWindowDays(days)}>{days}D</button>)}</div></section>
    <div className="stat-summary-grid">
      <Panel className="stat-hero-panel"><div className="section-heading compact"><div><p className="eyebrow">Last {windowDays} calendar days</p><h2>Recorded moments by quality</h2></div><span className="section-icon"><Activity size={19} /></span></div><div className="radar-wrap" role="img" aria-label={`Recorded moments by quality in the last ${windowDays} days. ${activitySummary}`}>{data.length ? <svg viewBox={`0 0 ${radarChart.width} ${radarChart.height}`} aria-hidden="true" focusable="false">{[0.25, 0.5, 0.75, 1].map((ring) => data.length > 2 ? <polygon key={ring} points={radarPolygon(() => radarChart.radius * ring)} fill="none" stroke="var(--line)" /> : <circle key={ring} cx={radarChart.x} cy={radarChart.y} r={radarChart.radius * ring} fill="none" stroke="var(--line)" />)}{data.map((item, index) => { const edge = radarPoint(index, radarChart.radius); const label = radarPoint(index, radarChart.radius + 29); return <g key={item.name}><line x1={radarChart.x} y1={radarChart.y} x2={edge.x} y2={edge.y} stroke="var(--line)" /><text x={label.x} y={label.y + 4} textAnchor={Math.abs(label.x - radarChart.x) < 8 ? "middle" : label.x < radarChart.x ? "end" : "start"} fill="var(--muted)" fontSize="11">{item.name}</text></g>; })}{data.length > 2 ? <polygon points={radarPolygon((activity) => (activity / radarMax) * radarChart.radius)} fill="var(--accent)" fillOpacity="0.18" stroke="var(--accent)" strokeWidth="2" /> : data.map((item, index) => { const point = radarPoint(index, (item.activity / radarMax) * radarChart.radius); return <circle key={item.name} cx={point.x} cy={point.y} r="5" fill="var(--accent)" />; })}</svg> : <p className="supportive-copy">Add a quality to see its recorded activity here.</p>}</div><Link className="trace-source-link" href={timelineHref({ ...currentRange, type: "activity" })}>Inspect all source records<ArrowUpRight size={14} /></Link></Panel>
      <div className="stat-callouts">
        <Panel><span className="callout-icon positive"><BarChart3 /></span><div><p className="eyebrow">Most recorded connection</p>{strongest ? <Link className="trace-link" href={timelineHref({ ...currentRange, type: "activity", statId: strongest.stat.id })}><h3>{strongest.stat.name}</h3></Link> : <h3>No activity yet</h3>}<p>{strongest ? `${strongest.activity.length} recorded ${strongest.activity.length === 1 ? "moment" : "moments"} across ${strongest.connectedGoals.length} connected ${strongest.connectedGoals.length === 1 ? goalTerm.toLowerCase() : terms.goals.toLowerCase()}.` : `Connect a quality to a ${goalTerm.toLowerCase()}; its permanent activity records will appear here.`}</p></div></Panel>
        ...
        <Panel><span className="callout-icon"><Sparkles /></span><div><p className="eyebrow">Quietest current connection</p>{quietest ? <Link className="trace-link" href={timelineHref({ ...currentRange, type: "activity", statId: quietest.stat.id })}><h3>{quietest.stat.name}</h3></Link> : <h3>Your choice</h3>}<p>{quietest ? `${quietest.activity.length} recorded ${quietest.activity.length === 1 ? "moment" : "moments"} across its connected ${terms.goals.toLowerCase()}. This is context, not a judgement or target.` : "Every quality can be shaped around what matters to you."}</p></div></Panel>
      </div>
    </div>

    <section className="stats-section">
      <div className="section-heading"><div><p className="eyebrow">{areaTerm} context</p><h2>Where activity was recorded</h2></div></div>
      <Panel>
        ...
        {areaRows.length ? <div className="activity-list" role="list" aria-label={`Recorded moments and logged time by life ${areaTerm.toLowerCase()} in the last ${windowDays} calendar days`}>{areaRows.map(({ area, activity, activeGoals, durationMinutes }) => <div key={area.id} role="listitem"><i className="area-dot" style={{ background: area.color }} /><div><Link className="trace-link" href={timelineHref({ ...currentRange, type: "activity", areaId: area.id })}><strong>{area.name}</strong></Link><small>{activeGoals} active {activeGoals === 1 ? goalTerm.toLowerCase() : terms.goals.toLowerCase()} · {formatDuration(durationMinutes)}</small></div><b>{activity} {activity === 1 ? "moment" : "moments"}</b></div>)}</div> : <p className="supportive-copy">Add a {goalTerm.toLowerCase()} to a life {areaTerm.toLowerCase()} to begin seeing context. Equal activity across {terms.areas.toLowerCase()} is not expected or required.</p>}
        {areaRows.length ? <p className="supportive-copy">Counts use unique permanent records from the last {windowDays} local calendar days. An action that updates a measurement at the same instant is counted once. Time includes only durations explicitly recorded on completions. {activeGoalAreasWithActivity} of {activeGoalAreas.length} {terms.areas.toLowerCase()} with active {terms.goals.toLowerCase()} had activity; this is context, not a target.</p> : null}
      </Panel>
    </section>

    <section className="stats-section">
      <div className="section-heading"><div><p className="eyebrow">Activity over time</p><h2>Quality history</h2></div></div>
      <Panel>
        <div className="quality-history-wrap" role="img" aria-label={`Recorded moments over the last ${windowDays} calendar days. ${historySummary}`}>
          ...
          </> : <p className="supportive-copy">Add a quality to see its recorded history here.</p>}
        </div>
        <p className="supportive-copy">Each line counts permanent records connected to that quality in local-calendar buckets. It is a history view, not a target.</p>
      </Panel>
    </section>

    <section className="stats-section"><div className="section-heading"><div><p className="eyebrow">Individual paths</p><h2>Quality activity</h2></div></div><div className="stat-card-grid">{recentByStat.map(({ stat, recent, connectedGoals, activity }) => <Panel key={stat.id} className="stat-card"><div className="stat-card-head"><span style={{ color: stat.color, background: `${stat.color}18` }}><DynamicIcon name={stat.icon} size={21} /></span><div><p>Recorded moments</p><strong>{activity.length}</strong></div></div><h3>{stat.name}</h3><div className="stat-context-copy"><span>{activity.length} in {windowDays} days</span><small>{connectedGoals.length} connected {connectedGoals.length === 1 ? goalTerm.toLowerCase() : terms.goals.toLowerCase()}</small></div>{connectedGoals.length ? <div className="stat-goal-links" aria-label={`${terms.goals} connected to ${stat.name}`}>{connectedGoals.slice(0, 3).map((goal) => <Link key={goal.id} href={`/goals/${goal.id}`}>{goal.title}</Link>)}{connectedGoals.length > 3 ? <small>+{connectedGoals.length - 3} more</small> : null}</div> : null}<div className="stat-recent"><small>Recent source records in this window</small>{recent.length ? recent.map((moment) => <div key={moment.id}><Link href={moment.href}>{moment.title}</Link><strong>{formatDate(moment.at)}</strong></div>) : <p>No permanent records in this window yet.</p>}</div><Link className="trace-source-link" href={timelineHref({ ...currentRange, type: "activity", statId: stat.id })}>Inspect all source records<ArrowUpRight size={14} /></Link><Link href="/settings#stats" className="card-arrow" aria-label={`Customise ${stat.name}`}><ArrowUpRight size={16} /></Link></Panel>)}</div></section>
  </div>;
}
