...
import { Activity, ArrowUpRight, BarChart3, CalendarRange, Minus } from "lucide-react";
  ...
  return <div className="instrument-route instrument-route-stats">
    ...
    <div className="stat-summary-grid">
      ...
      <div className="stat-callouts">
        ...
        <Panel><span className="callout-icon"><Minus /></span><div><p className="eyebrow">Quietest current connection</p>{quietest ? <Link className="trace-link" href={timelineHref({ ...currentRange, type: "activity", statId: quietest.stat.id })}><h3>{quietest.stat.name}</h3></Link> : <h3>Your choice</h3>}<p>{quietest ? `${quietest.activity.length} recorded ${quietest.activity.length === 1 ? "moment" : "moments"} across its connected ${terms.goals.pluralLower}. This is context, not a judgement or target.` : `Every ${terms.stats.singularLower} can be shaped around what matters to you.`}</p></div></Panel>
      </div>
    </div>
