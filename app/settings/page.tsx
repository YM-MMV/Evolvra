...
import type { Area, LifeStat } from "@/lib/types";
...
export default function SettingsPage() {
  ...
  const [active, setActive] = useState(() => typeof window !== "undefined" && (window.location.hash === "#stats" || window.location.hash === "#areas") ? "structure" : "profile");
  ...
  const exportCsv = () => {
    const rows = [["date", "type", "title", "detail", "goal", "area"], ...state.timeline.map((event) => [event.at, event.type, event.title, event.detail, state.goals.find((goal) => goal.id === event.goalId)?.title ?? "", state.areas.find((area) => area.id === event.areaId)?.name ?? ""])];
  ...
  const onImport = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 5_000_000) { setMessage("That backup is too large to import safely. Choose a file under 5 MB."); event.target.value = ""; return; }
  ...
  const eraseEverything = async () => {
    ...
    try {
      if (user) {
        ...
        const { error: snapshotError } = await supabase.from("workspace_snapshots").delete().eq("user_id", user.id);
      ...
      resetWorkspace();
      setDangerOpen(false);
      setMessage("The workspace was erased successfully.");
    } catch (error) {
      setMessage(error instanceof Error ? `Erase failed: ${error.message}` : "Erase failed. Your local workspace was kept so you can retry.");
    } finally {
      setErasing(false);
    }
  };
  ...
  return <div>
    ...
    <div className="settings-layout">
      ...
      <div className="settings-content">
        ...
        {active === "structure" && <div className="settings-stack">
          ...
          <Panel id="stats"><SettingsHead eyebrow="Personal qualities" title={term.stats} description="Use qualities to group the goals and actions that develop different parts of your life." action={<Button onClick={() => openEdit({ kind: "stat" })}><Plus size={15} /> Add stat</Button>} /><div className="custom-list">{state.stats.map((stat) => <div key={stat.id} className={stat.archived ? "archived" : ""}><span className="custom-icon" style={{ color: stat.color, background: `${stat.color}18` }}><DynamicIcon name={stat.icon} /></span><div><strong>{stat.name}</strong><small>{stat.archived ? "Archived" : "Available for goal connections"}</small></div><Pill>{state.goals.filter((goal) => goal.statIds.includes(stat.id)).length} connected</Pill><button className="icon-button" aria-label={`Edit ${stat.name}`} onClick={() => openEdit({ kind: "stat", value: stat })}><Edit3 size={15} /></button><button className="icon-button danger" aria-label={`Remove ${stat.name}`} onClick={() => removeStat(stat.id)}><Trash2 size={15} /></button></div>)}</div></Panel>
        </div>}
    ...
    <Modal open={dangerOpen} onClose={() => { if (!erasing) setDangerOpen(false); }} eyebrow="Permanent action" title="Erase your Evolvra workspace?"><div className="danger-confirm"><span><Trash2 /></span><p>This removes all local information and, if connected, your cloud snapshot and account. Your local copy is kept if cloud deletion fails so you can retry.</p><div className="button-row end"><Button variant="ghost" disabled={erasing} onClick={() => setDangerOpen(false)}>Keep my data</Button><Button variant="danger" disabled={erasing} onClick={eraseEverything}>{erasing ? "Erasing…" : "Erase everything"}</Button></div></div></Modal>
  </div>;
}
