...
export default function QuestsPage() {
  ...
  const quests = useMemo(() => state.goals.flatMap((goal) => goal.quests.map((quest) => ({ quest, goal }))).filter(({ quest, goal }) => {
    const available = !quest.completed && (!quest.dueDate || quest.dueDate <= today);
    const stateMatch = view === "all" || (view === "completed" ? quest.completed : available);
  ...
  }).sort((a, b) => Number(a.quest.completed) - Number(b.quest.completed) || (a.quest.dueDate ?? "9999").localeCompare(b.quest.dueDate ?? "9999")), [areaId, query, state.goals, today, view]);

  const grouped = quests.reduce<Record<string, typeof quests>>((groups, item) => {
    const key = item.quest.completed ? "Completed" : item.quest.dueDate && item.quest.dueDate <= today ? "Today & overdue" : item.quest.dueDate ? "Upcoming" : "Anytime";
  ...
  return <div>
    ...
    {!quests.length ? <EmptyState icon={<CheckCircle2 />} title="No quests in this view" body="A clear list can be a sign of focus. Add the next useful action when you are ready." /> : <div className="quest-groups">{["Today & overdue", "Upcoming", "Anytime", "Completed"].filter((group) => grouped[group]).map((group) => <section key={group}><div className="section-heading compact"><div><p className="eyebrow">{group}</p><h2>{group === "Completed" ? "Recorded actions" : `${group} quests`}</h2></div><span className="count-badge">{grouped[group].length}</span></div><Panel className="quest-board-list">{grouped[group].map(({ quest, goal }) => { const area = getArea(state, goal.areaId); const available = !quest.completed && (!quest.dueDate || quest.dueDate <= today); return <div key={`${goal.id}-${quest.id}`} className={quest.completed ? "quest-board-row completed" : "quest-board-row"}><button className="quest-check large" aria-label={quest.completed ? `${quest.title} completed` : available ? `Complete ${quest.title}` : `${quest.title} is available ${formatDate(quest.dueDate!)}`} onClick={() => completeQuest(goal.id, quest.id)} disabled={!available}>{quest.completed ? <Check size={18} /> : <Circle size={19} />}</button><div className="quest-board-copy"><strong>{quest.title}</strong><Link href={`/goals/${goal.id}`}><i style={{ background: area?.color }} />{goal.title}</Link></div><div className="quest-tags">{quest.repeat !== "none" && <Pill><Repeat2 size={12} /> {quest.repeat}</Pill>}{quest.dueDate && <Pill><CalendarDays size={12} /> {formatDate(quest.dueDate)}</Pill>}{quest.durationMinutes ? <Pill>{quest.durationMinutes} min</Pill> : null}</div></div>; })}</Panel></section>)}</div>}
  </div>;
}
